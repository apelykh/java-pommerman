...
public class ModifiedAdvancedHeuristic extends StateHeuristic {

    private BoardStats rootBoardStats;
    private Random random;

    public ModifiedAdvancedHeuristic(GameState root, Random random) {
        this.random = random;
    ...
    public static class BoardStats
    {
        ...
        // This factor will effect the score by decreasing it or increasing it, depending on how close the player
        // is from the closest wall. It will decrease it if the agent is close to the wall and vice-versa.
        double FACTOR_WALL_APPROACHING = 0.2;
        ...
        double score(BoardStats futureState)
        {
            ...
            //int currentBoardSize = calcEncroachment(this.tick);
            //int playerDisFromWall = calcDistFromWall(myPosition.x, myPosition.y, currentBoardSize);
            //System.out.println(playerDisFromWall);

            double defScore = (diffSafeDirections / 4.0) * FACTOR_SAFE_DIRECTIONS
            ...
            if(isWithinWallIncoming(this.tick + 2)) {

                // int distToWall = calcDistFromWall(myPosition.x, myPosition.y, currentBoardSize);

                int currentBoardSize = calcEncroachment(this.tick);
                int maxDistToWall = calcDistFromWall(5, 5, currentBoardSize);

                int agentDistToWall = calcDistFromWall(myPosition.x, myPosition.y, currentBoardSize);

                double normAgentDistToWall = (double) agentDistToWall / (double) maxDistToWall;

                normAgentDistToWall = -(1 - normAgentDistToWall);


                // Factors that will be 0, to get 0.2 for dist to wall factor.
//                double FACTOR_CANKICK = 0.05;
//                double FACTOR_BLAST = 0.05;
//                double FACTOR_NEAREST_POWERUP = 0.05;
//                double FACTOR_WOODS = 0.05;
                ...
                score = (diffSafeDirections / 4.0) * FACTOR_SAFE_DIRECTIONS
                        + (diffDirectionsInRangeOfBomb / 4.0) * FACTOR_BOMB_DIRECTIONS
                        + (diffEnemies / 3.0) * FACTOR_ENEMY
                        + diffTeammates * FACTOR_TEAM
                        + (diffDistanceToNearestEnemy / 10.0) * FACTOR_ENEMY_DIST
                        + (diffWoods / maxWoods) * FACTOR_WOODS * 0
                        + diffCanKick * FACTOR_CANKICK * 0
                        + (diffBlastStrength / maxBlastStrength) * FACTOR_BLAST * 0
                        //+ diffAdjacentEnemy * FACTOR_ADJ_ENEMY
                        + (diffDistanceToNearestPowerUp / 10.0) * FACTOR_NEAREST_POWERUP * 0
                        + (normAgentDistToWall * FACTOR_WALL_APPROACHING);

                // System.out.println("Curr Board Size: " + currentBoardSize +
                //         "  Agent dist to wall: " + agentDistToWall +
                //         "  Max Dist to wall: " + maxDistToWall +
                //         "  Norm Agent To Wall: " + normAgentDistToWall +
                //         "  score: " + score +
...
//                    + (diffDistanceToNearestPowerUp / 10.0) * FACTOR_NEAREST_POWERUP;
        ...
        // Ensure to pass in t - 2 (where 2 is how far ahead we want want to consider the wall coming in).
        private boolean isWithinWallIncoming(int t) {
            int tickDiff = t - 500;

            //  E.g. 75 % 75 = 0            76 % 75 = 1
            int tickDiffRem = tickDiff % 75;

            // For now, for 2 to be larger, we need to add tickDiffRem == 3, etc. for every one increase in 2.
            if(tickDiffRem == 0 || tickDiffRem == 1 || tickDiffRem == 2){
                return true;
            }
        ...
        private int calcDistFromWall(int x, int y, int curBoardSize) {
            x  = x + 1;
            y = y + 1;
            int boardDiff = Types.BOARD_SIZE - curBoardSize;
            int centToWall = 5 - boardDiff;

            int horiDist = centToWall - abs(x - (centToWall + 1)) + boardDiff;
            int vertDist = centToWall - abs(y - (centToWall + 1)) + boardDiff;

            return min(horiDist, vertDist);

        }

        private int calcEncroachment(int t) {
            // t = t - 2;
            if (t >= 500) {
                double timeSince = t - 500.0; // 575 - 500 = 75
                double encroachment = timeSince / 75.0;
                int encroachmentInt = (int) encroachment;
                //System.out.println(Types.BOARD_SIZE - encroachmentInt- 1);
                return Types.BOARD_SIZE - encroachmentInt - 1;
            }
            else {
                return Types.BOARD_SIZE;
            }
        }
        private int evaluateBoundary(BoardStats bs) {
            return Types.BOARD_SIZE - Types.BOARD_SIZE;
        }
